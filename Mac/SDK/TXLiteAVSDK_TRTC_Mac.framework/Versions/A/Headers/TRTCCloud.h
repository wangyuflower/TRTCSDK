...
/// 设置驱动回调的队列
///
/// 默认会采用 Main Queue 也就是说，如果您不指定 delegateQueue，那么直接在 TRTCCloudDelegate 的回调函数中操作 UI 界面将是安全的。
...
/**
 * 1.3 请求跨房连麦
 * @param param JSON 字符串连麦参数，其中，
 *              roomId：int 连麦房间号；
 *              userId: String 被连麦帐号；
 *              sign: String   跨房连麦签名，加密内容为 TC_ConnRoomSig，由第三方后台加密生成，是否使用由 SPEAR 配置决定
 **/
 ...
 * @param view 指定渲染控件所在的父控件，SDK 会在 view 内部创建一个等大的子控件用来渲染本地摄像头的视频画面
 */
 ...
 * @param view 指定渲染控件所在的父控件，SDK 会在 view 内部创建一个等大的子控件用来渲染远端画面
...
/**
 * 4.3 设置摄像头缩放因子（焦距）
 * @param distance 取值范围为1 - 5 ，当为1的时候为最远视角（正常镜头），当为5的时候为最近视角（放大镜头），这里最大值推荐为5，超过5后视频数据会变得模糊不清
 */
 ...
 * @param beautyLevel 美颜级别，取值范围0 - 9； 0 表示关闭，1 - 9值越大，效果越明显
 * @param whitenessLevel 美白级别，取值范围0 - 9； 0 表示关闭，1 - 9值越大，效果越明显
 * @param ruddinessLevel 红润级别，取值范围0 - 9； 0 表示关闭，1 - 9值越大，效果越明显
 */
 ...
 * @param rect 水印相对于编码分辨率的归一化坐标，x, y, width, height 取值范围 0 - 1；height 不用设置，SDK 内部会根据水印宽高比自动计算 height
 */
...
/**
 * 8.1 启用视频自定义采集模式，即放弃 SDK 原来的视频采集流程，改用 sendCustomVideoData 向 SDK 塞入自己采集的视频画面
 * @param enable 是否启用
 */
- (void)enableCustomVideoCapture:(BOOL)enable;

/**
 * 8.2 发送自定义的 SampleBuffer
 * @note SDK 内部有帧率控制，发送太快会自动丢帧，超时则会重发最后一帧
 * @param frame 视频数据，支持 PixelBuffer NV12，BGRA，I420格式数据
 */
- (void)sendCustomVideoData:(TRTCVideoFrame *)frame;

/**
 * 8.3 设置本地视频的自定义渲染回调
 * @note 设置此方法后，SDK 内部会把采集到的数据回调出来，SDK 跳过自己原来的渲染流程，您需要自己完成画面的渲染
 * @param delegate    自定义渲染回调
 * @param pixelFormat 指定回调的像素格式，目前仅支持 TRTCVideoPixelFormat_I420
 * @param bufferType  SDK 为了提高回调性能，提供了两种 PixelBuffer 格式，一种是 iOS 原始的（TRTCVideoBufferType_PixelBuffer），一种是经过内存整理的（TRTCVideoBufferType_NSData）
 * @return 0：成功；<0：错误
 */
...
/**
 * 8.4 设置远端视频的自定义渲染回调
 * @note 设置此方法后，SDK 内部会把远端的数据解码后回调出来，SDK 跳过自己原来的渲染流程，您需要自己完成画面的渲染
 ...
 * 8.6 发送自定义的音频数据 (iOS)
 * @param frame 音频数据
 * @note frame.timestamp 未使用，SDK 内部会重打时间戳，为了保证时间戳正常，建议根据发送的音频数据量来控制 sendCustomAudioData 的调用间隔
 */
 ...
 *
 * @note 本接口有以下限制：
 *       1. 发送消息到房间内所有用户，每秒最多能发送 30 条消息
 *       2. 每个包最大为 1 KB，超过则很有可能会被中间路由器或者服务器丢弃
 *       3. 每个客户端每秒最多能发送总计 8 KB 数据
 *
 ...
 *       2. 每个包最大为1KB，若发送大量数据，会导致视频码率增大，可能导致视频画质下降甚至卡顿（与 sendCustomCmdMsg 共享限制）
 ...
 * 11.5 开始进行麦克风测试
 * 
 * 该方法测试麦克风是否能正常工作，volume 的取值范围为 0 - 100。
 */
...
/**
 * 12.1 启动 CDN 发布：通过腾讯云将当前房间的音视频流发布到直播 CDN 上
 *
 * 由于 TRTC 的线路费用是按照时长收费的，并且房间容量有限（< 1000人）。
 * 当您有大规模并发观看的需求时，将房间里的音视频流发布到低成本高并发的直播 CDN 上是一种较为理想的选择。
 * 目前支持两种发布方案：
 *
...
/**
 * 12.3 启动(更新)云端的混流转码：通过腾讯云的转码服务，将房间里的多路画面叠加到一路画面上
 *
 * <pre>
 * 【画面1】=> 解码 => =>
 *                         \
